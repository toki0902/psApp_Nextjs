        ...
        const onPassCheck = async (newTitle: string) => {
          if (data.edit) {
            const res = await fetch(
              `${process.env.NEXT_PUBLIC_ROOT_URL ?? window.location.origin}/v1/api/users/${data.edit.ownerId}/playlists/id/${data.edit.playlistId}`,
              {
                method: "PATCH",
      ...
      return async () => {
        const onPassCheck = async () => {
          if (data.deletePlaylist) {
            const res = await fetch(
              `${process.env.NEXT_PUBLIC_ROOT_URL ?? window.location.origin}/v1/api/users/${data.deletePlaylist.ownerId}/playlists/id/${data.deletePlaylist.playlistId}`,
              { method: "DELETE" },
            );
      ...
      return async () => {
        const onPassCheck = async (addPlaylistIds: string[]) => {
          if (data.addFavorite) {
            ...
            const res = await fetch(
              `${process.env.NEXT_PUBLIC_ROOT_URL ?? window.location.origin}/v1/api/users/${data.addFavorite.userPlaylistsInfo[0].ownerId}/playlists/videos`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
      ...
      return async () => {
        const onPassCheck = async () => {
          if (data.deleteFromPlaylist) {
            const res = await fetch(
              `${process.env.NEXT_PUBLIC_ROOT_URL ?? window.location.origin}/v1/api/users/${data.deleteFromPlaylist.ownerPlaylistInfo.ownerId}/playlists/id/${data.deleteFromPlaylist.ownerPlaylistInfo.playlistId}/videos/${data.deleteFromPlaylist.memberId}`,
              { method: "DELETE" },
            );
